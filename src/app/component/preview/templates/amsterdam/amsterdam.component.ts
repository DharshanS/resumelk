...
export class AmsterdamComponent implements OnInit {
  ...
  @ViewChild("amsterdam", { static: false }) amsterdamPage: ElementRef;
  amsterdamPage2 = true;
  templateData: any;
  ...
  ngAfterViewInit(): void {
    ...
    setTimeout(() => {
      if (this.urilityService.isObjectIsNotNull(this.resumeService.resumeComponents)) {
        //  this.amsterdamPage2 = this.urilityService.templatePageBreak(this.amsterdamPage, this.page2Class);
        this.loadComponent();
      }
    }, 1000)
  ...
  loadComponent() {
    this.componentList = this.urilityService.pageElementsHTML(this.amsterdamPage);
    console.log(this.amsterdamPage)
    // let adItem = this.ads[0];
    //adItem.data = this.urilityService.pageElementsHTML(this.amsterdamPage);
